...
export const checkOtpRestriction = async (email) => {
    try {
        ...
        if (await redis.get(`otp_spam_lock:${email}`)) {
            throw new ValidationError(`Too many OTP request! Please wait one hour before requesting`);
        }
    ...
    try {
        ...
        if (storedOtp != otp) {
            if (failedAttempts >= 2) {
                ...
                throw new ValidationError(`Too many failed attempt, Try later after 30 min!`);
            }
